@icon("uid://dnduw2kvvfx63")
@tool
class_name Node3DTween extends TweenComponent
...
@export var transform3D_tween : Transform3DTween:
	set(new_tween):
		transform3D_tween = new_tween
...
func _tween_values(forward : bool = true) -> void:
	if !use_custom_curve:
		if transform3D_tween != null: transform3D_tween.tween_properties(tween, tween_duration, affected_node, forward)
	else:
		if transform3D_tween != null: transform3D_tween.custom_tween_properties(tween, tween_duration, affected_node, transition_curve, forward)


## Sets the current transform values to the [member affected_node]
func set_current_values() -> void:
	if transform3D_tween != null: transform3D_tween.set_current_values(affected_node)
...
func _set_reset_values() -> void:
	if transform3D_tween != null: transform3D_tween._set_reset_values(affected_node)
...
func _reset_values() -> void:
	if loop and tween.is_running():
		tween.stop()
	if transform3D_tween != null: transform3D_tween._reset_values(affected_node)
