...
func do_transform_tween(affected_node : Node) -> void:
	...
	if start_position.x != end_position.x:
		tween.tween_property(
			affected_node,
			"position:x",
			end_position.x if end_position.x != INF else original_position.x,
			tween_duration
			).from(
				start_position.x if start_position.x != INF else original_position.x
				)
	if start_position.y != end_position.y:
		tween.tween_property(
			affected_node,
			"position:y",
			end_position.y if end_position.y != INF else original_position.y,
			tween_duration
			).from(
				start_position.y if start_position.y != INF else original_position.y
				)
	...
	if start_rotation != end_rotation:
		tween.tween_property(
			affected_node,
			"rotation_degrees",
			end_rotation if end_rotation != INF else original_rotation,
			tween_duration
			).from(
				start_rotation if start_rotation != INF else original_rotation
				)
	...
	if start_scale.x != end_scale.x:
		tween.tween_property(
			affected_node,
			"scale:x",
			end_scale.x if end_scale.x != INF else original_scale.x,
			tween_duration
			).from(
				start_scale.x if start_scale.x != INF else original_scale.x
				)
	if start_scale.y != end_scale.y:
		tween.tween_property(
			affected_node,
			"scale:y",
			end_scale.y if end_scale.y != INF else original_scale.y,
			tween_duration
			).from(
				start_scale.y if start_scale.y != INF else original_scale.y
				)
...
# hi precum :)
#subscribe to chopiethecat on youtube C:
